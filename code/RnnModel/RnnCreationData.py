...
class ModelData:
    ...
    def data_common(self, model_name: str, con_x, con_y, height=30, width=30):  # width=w2, height=h1
        fn = f'{model_name}_{self.stock_code}'
        ...
        """ 判断是否调用以前数据 """
        if self._month:
            fp = f'{self.root}/data/{self._month}/train_data/'
            data_x = np.load(f'{fp}{fn}_x.npy', allow_pickle=True)
            data_y = np.load(f'{fp}{fn}_y.npy', allow_pickle=True)
        ...
        # 新数据储存
        np.save(f'{self.root}/data/{self.months}/train_data/{fn}_x.npy', data_x)
        np.save(f'{self.root}/data/{self.months}/train_data/{fn}_y.npy', data_y)
...
class TrainingDataCalculate(ModelData):
    ...
    def __init__(self, Stock: str, months: str, _start: str, _month):
    ...
    def first_calculate(self):
        ...
        self.data_15m = ResampleData.resample_1m_data(self.data_1m, self.freq)
        self.data_15m = SignalMethod.signal_by_MACD_3ema(self.data_15m, self.data_1m)
    ...
    def third_calculate(self):
        ...
        vol_parser = ['volume', Cycle1mVolMax1, Cycle1mVolMax5,
                      Daily1mVolMax1, Daily1mVolMax5, Daily1mVolMax15,
                      Bar1mVolMax1, Bar1mVolMax5, 'EndDaily1mVolMax5']  # 成交量乘以参数，数据相似化
    ...
    def save_15m_data(self):
        ...
        record_end_SignalTimes = self.data_15m.iloc[-1]['SignalTimes']
        record_end_SignalStartTime = self.data_15m.iloc[-1]['SignalStartTime'].strftime('%Y-%m-%d %H:%M:%S')
        record_end_next_start = self.data_15m.drop_duplicates(subset=[SignalTimes]).tail(6).iloc[0]['date'].strftime(
            '%Y-%m-%d %H:%M:%S')

        records = ReadSaveFile.read_json(self.months, self.stock_code)
        records['RecordEndDate'] = record_end_date
        records['RecordEndSignal'] = record_end_signal
        records['RecordEndSignalTimes'] = record_end_SignalTimes
        records['RecordEndSignalStartTime'] = record_end_SignalStartTime
        records['RecordNextStartDate'] = record_end_next_start
...
class RMTrainingData:
    ...
    def __init__(self, months: str, _start: str, _month):
        self.months = months
        self._date = _start
        self._month = _month

    def update_records(self, ids):
        # 在数据库中更新记录的逻辑
        # 使用参数化查询来防止 SQL 注入
    ...
    def single_stock(self, Stock):
        calculation = TrainingDataCalculate(Stock, self.months, self._date, self._month)
        calculation.calculation_single()

    def all_stock(self):
        records = LoadRnnModel.load_train_record()
        records_copy = records[records['ParserMonth'] == self.months]
        ...
        if not records.empty:
            ...
            for i, (index, row) in zip(range(shapes), records.iterrows()):
                stock_ = row['name']
                id_ = row['id']
                print(f'\n计算进度：\n剩余股票: {(shapes - i)} 个; 总股票数: {shapes}个;\n当前股票：{stock_};')

                try:
                    run = TrainingDataCalculate(stock_, self.months, self._date, self._month)
...
if __name__ == '__main__':
    ...
    running = RMTrainingData(month_, start_, _month)
    running.all_stock()
