...
class ModelData(Parsers):
    ...
    def read_1m(self):
        ...
        if self.monitor:
            ...
            self.data_1m = pd.concat([data_1m, data_], ignore_index=True)

        else:
            date_ = self.check_date + pd.Timedelta(days=1)
            data_1m = data_1m[data_1m['date'] < date_]
            self.data_1m = data_1m.drop_duplicates(subset=['date']).reset_index(drop=True)
    ...
    def Bar1mVolumeMax(self, x, num: int):
        st = x + pd.Timedelta(minutes=-15)
        ed = x
        max_vol = self.data_1m[(self.data_1m['date'] > st) &
    ...
    def daily_data(self):
        ...
        data.loc[:, 'date'] = pd.to_datetime(data['date']) + pd.Timedelta(minutes=585)
    ...
    def first_15m(self):
        ...
        Time15m = self.data_15m.drop_duplicates(subset=[SignalTimes]).tail(6).iloc[0]['date'].date()

        sql = f'''update {LoadRnnModel.db_rnn}.{LoadRnnModel.tb_run_record} 
        set Time15m = '{Time15m}' where id = {self.stock_id};'''
    ...
    def second_15m(self):
        ...
        for c in li:
            self.data_15m.loc[:, c] = round(self.data_15m[c] * self.data_15m[DailyVolEmaParser])
...
class UpdateData(Parsers):
    ...
    def update_RecordRun(self):
        ...
        RenewDate = '{pd.Timestamp('today').date()}'
