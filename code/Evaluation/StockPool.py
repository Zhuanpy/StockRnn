...
def open_data_file():
    import os
    start_directory = r'E:\Python\Project\Stock\Project_01\code\Dl_Strategy\Stock_RNN\data\output\stock_pool'
...
class ScoreStockPool(TrendDistinguishModel):
    ...
    def board_trends(self, code: str):
        ...
        try:
            data, label, score = self.distinguish_1m(code_=code, freq='120m', returnFreq=True, date_=None)

            data.loc[:, 'close'] = mfl.data2normalization(data['close'])
    ...
    def analysis_Industry(self):
        ...
        pool = StockPoolData.load_StockPool()
        poolB = pool.drop_duplicates(subset=['IndustryCode']).reset_index(drop=True)

        for index in poolB.index:

            self.codeB = poolB.loc[index, 'IndustryCode']
            self.nameB = poolB.loc[index, 'Industry']

            self.dataB, self.scoreB = self.board_trends(self.codeB)
            self.dataF, self.scoreF = self.funds_trends(self.codeB)

            # 更新数据
            ids = tuple(pool[pool['IndustryCode'] == self.codeB]['id'])
            if len(ids) == 1:
                where = f'''id = {ids[0]}'''
    ...
    def ScoreFundsAwkward(self):
        ...
        for index in pool.index:
            ...
            if stock_name in list(bb['stock_name']):
...
class UpdateData:
    ...
    def data_current_trade(cls, file_):
        root_ = file_root()
        data = pd.read_excel(f'{root_}/data/output/stock_pool/{file_}.xls', sheet_name='table')
        data.loc[:, '成交日期'] = pd.Timestamp('today').strftime('%Y%m%d')
        data.loc[:, ['信号编号', '交易市场', '股东帐户', ]] = None
        data.loc[:, '股票余额'] = data['成交数量']

        data.loc[:, ['止损价', '手续费', '印花税', '其他杂费', '发生金额', '真实操作', '撤单数量']] = 0
...
class UpdateFakeStock(UpdateTradeHistory):  # 更新模拟账户交易数据
    ...
    def update_position(self, file_='position_fake'):
        ...
        try:
            ...
            if data.shape[0]:
                ...
                for index in data.index:
                    ...
                    positionNum = data.loc[index, '股票余额']
                    ...
                    try:
                        ...
                        Position = 1, 
                        TradeMethod = 1,
                        PositionNum = {positionNum} where id = {id_};'''
                        StockPoolData.pool_execute_sql(sql)
    ...
    def update_trade_record(self, data, current: bool):
        ...
        for index in data.index:
            ...
            predict = PredictionCommon(Stock=self.code, months=self.months, monitor=monitor, check_date=date_)
    ...
    def update_history_trade(self, file_='history_fake'):
        ...
        try:
            ...
            self.update_trade_record(data, current=False)  # 更新数据
    ...
    def update_current_trade(self, file_='current_fake'):
        ...
        try:
            ...
            self.update_trade_record(data, current=True)
